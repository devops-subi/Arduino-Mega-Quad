...
void setup(){
  ...
  DDRL |= B00111100;                                                        //Configure digital port 47, 46,45 and 44 as output.
  ...
  for (cal_int = 0; cal_int < 1250 ; cal_int ++){                           //Wait 5 seconds before continuing.
    PORTL |= B00111100;                                                     //Set digital port 47, 46, 45 and 44 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTL &= B11000011;                                                     //Set digital port 47, 46, 45 and 44 low.
...
void loop(){
  ...
  PORTL |= B00111100;                                                       //Set digital outputs 47,46,45 and 44 high.
  ...
  while(PORTL >= 16){                                                       //Stay in this loop until output 47,46,45 and 44 are low.
    esc_loop_timer = micros();                                              //Read the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTL &= B11111011;                //Set digital output 47 to low if the time is expired.
    if(timer_channel_2 <= esc_loop_timer)PORTL &= B11110111;                //Set digital output 46 to low if the time is expired.
    if(timer_channel_3 <= esc_loop_timer)PORTL &= B11101111;                //Set digital output 45 to low if the time is expired.
    if(timer_channel_4 <= esc_loop_timer)PORTL &= B11011111;                //Set digital output 44 to low if the time is expired.
  }
}
