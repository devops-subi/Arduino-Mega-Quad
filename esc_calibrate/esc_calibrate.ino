...
void setup(){
  ...
  DDRL |= B11110000;                                                                    //Configure digital port 45, 44, 43 and 42 as output.
  DDRB |= B01000000;                                                                    //Configure digital port 12 as output.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        PORTL |= B00111100;                                                             //Set digital poort 4, 5, 6 and 7 high.
        delayMicroseconds(1000);                                                        //Wait 1000us.
        PORTL &= B11000011;                                                             //Set digital poort 4, 5, 6 and 7 low.
...
void esc_pulse_output(){
  zero_timer = micros();
  PORTL |= B00111100;                                            //Set port 45, 44, 43 and 42 high at once
  timer_channel_1 = esc_1 + zero_timer;                          //Calculate the time when digital port 45 is set low.
  timer_channel_2 = esc_2 + zero_timer;                          //Calculate the time when digital port 44 is set low.
  timer_channel_3 = esc_3 + zero_timer;                          //Calculate the time when digital port 43 is set low.
  timer_channel_4 = esc_4 + zero_timer;                          //Calculate the time when digital port 42 is set low.

  while(PORTL >= 16){                                            //Execute the loop until digital port 45 to 42 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTL &= B11111011;     //When the delay time is expired, digital port 45 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTL &= B11110111;     //When the delay time is expired, digital port 44 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTL &= B11101111;     //When the delay time is expired, digital port 43 is set low.
    if(timer_channel_4 <= esc_loop_timer)PORTL &= B11011111;     //When the delay time is expired, digital port 42 is set low.
  }
}
